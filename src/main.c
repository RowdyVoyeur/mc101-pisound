...
// Allow main.c to trigger the pipe check inside render.c
extern void update_overlay_data(void);
...
static config_params_s initialize_config(int argc, char *argv[], char **preferred_device,
  ...
  for (int i = 1; i < argc; i++) {
    if (SDL_strcmp(argv[i], "--list") == 0) exit(m8_list_devices());
  ...
  if (TARGET_OS_IOS == 1) conf.init_fullscreen = 1;
  config_read(&conf);
  return conf;
}
...
SDL_AppResult SDL_AppIterate(void *appstate) {
  ...
  // Check for nanoKontrol pipe data every frame
  update_overlay_data();

  switch (ctx->app_state) {
  case INITIALIZE: break;
  case WAIT_FOR_DEVICE: do_wait_for_device(ctx); break;
  case RUN: {
    if (ctx->app_suspended) return SDL_APP_CONTINUE;
  ...
  case QUIT: app_result = SDL_APP_SUCCESS; break;
  }
  return app_result;
}
...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char **argv) {
  ...
  if (ctx == NULL) return SDL_APP_FAILURE;

  *appstate = ctx;
  ctx->app_state = INITIALIZE;
  ctx->conf = initialize_config(argc, argv, &ctx->preferred_device, &config_filename);

  if (!renderer_initialize(&ctx->conf)) return SDL_APP_FAILURE;

  ctx->device_connected = m8_initialize(1, ctx->preferred_device);

  if (ctx->device_connected) SDL_Delay(500); 

  if (gamepads_initialize() < 0) return SDL_APP_FAILURE;
...
void SDL_AppQuit(void *appstate, SDL_AppResult result) {
  ...
  if (app) {
    if (app->app_state == WAIT_FOR_DEVICE) screensaver_destroy();
